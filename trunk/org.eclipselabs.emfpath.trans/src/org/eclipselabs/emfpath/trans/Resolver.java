...
public abstract class Resolver implements ILinkerResolver {
	...
	abstract void link(TransientLinkKey key, EObject target);

	abstract EObject resolve(TransientLinkKey key, EClass eClass);
	
	abstract <T extends EObject> T resolve(TransientLinkKey key, Class<T> clazz);
	...
	static class UnresolvingResolver extends Resolver {

		final static Resolver INSTANCE = new UnresolvingResolver();
